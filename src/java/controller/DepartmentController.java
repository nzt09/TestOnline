...
import sessionBean.DepartmentFacadeLocal;
...
public class DepartmentController implements Serializable {
    ...
    @EJB
    private sessionBean.DepartmentFacadeLocal ejbFacade;
    ...
    private DepartmentFacadeLocal getFacade() {
        return ejbFacade;
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        ...
        for (int i = 0; i < item.length; i++) {
            item[i].setLabel(((Department) item[i].getValue()).getName());
            item[i].setValue(((Department) item[i].getValue()).getId());
        }
        return item;
    }

    
    public SelectItem[] getItemsAvailableSelectManyD() {
        SelectItem[] item = JsfUtil.getSelectItems(ejbFacade.findAll(), false);
        for (int i = 0; i < item.length; i++) {
            item[i].setLabel(((Department) item[i].getValue()).getName());
